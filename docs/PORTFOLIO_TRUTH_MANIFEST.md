...
| Flagstone | "On the App Store · US and Canada · Sept 2026" | `content/deliverables.json:7-8` | **CONFIRMED-CORRECT** — re-verified 2026-09-17 (P1.A) against the App Store record itself, not a repo file: bundle `com.accessmap.app` → `trackId 6774709116`, `version 4.1.1`, `Free`, `releaseDate 2026-09-15T21:21:25Z`. Storefronts `us`/`ca` only (10 others return no result). The previous row ("App Store review submitted") became false on 2026-09-15 and stood for two days. Note AccessMap's `release/current.json` still reads `"status": "submitted_for_review"` and is now the stale source — do not re-derive Portfolio copy from it until it is updated. |
| Flagstone | "adoption and usage numbers are not yet meaningful" | `content/deliverables.json` body | CONFIRMED-CORRECT — the same lookup returns `userRatingCount 0` / `averageUserRating 0` in both storefronts. No adoption figure of any kind may be published; see `docs/IDENTITY_AND_CLAIM_CONTRACT.md` §20. |
