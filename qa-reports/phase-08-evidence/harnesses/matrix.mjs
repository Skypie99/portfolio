...
const out=path.resolve('work/evidence');fs.mkdirSync(out,{recursive:true});
...
fs.writeFileSync(out+'/manifest.json',JSON.stringify({sourceSha:'47c3d67c5fabd358c4cfc689d294c8395e65c494',sourceTree:'098ec9e6793e88d30172a65f3d35f67e26a3194f',allRoutes,routes,excluded:allRoutes.filter(r=>!routes.includes(r)),cells:cells.length,axeVersion:require('axe-core/package.json').version,instrumentSha256:crypto.createHash('sha256').update(instrument).digest('hex')},null,2));
