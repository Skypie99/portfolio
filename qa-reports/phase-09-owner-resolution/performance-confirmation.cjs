...
(async()=>{
  ...
  for(let rep=1;rep<=iterations;rep++) for(const device of devices) for(const route of routes) for(const target of (rep%2 ? targets : [...targets].reverse())) {
    ...
    await context.addInitScript(()=>{
      window.__lab = {lcp:[],shifts:[],longTasks:[],events:[],longAnimationFrames:[]};
      for(const [type,key] of [['largest-contentful-paint','lcp'],['layout-shift','shifts'],['longtask','longTasks'],['event','events'],['long-animation-frame','longAnimationFrames']]) {
